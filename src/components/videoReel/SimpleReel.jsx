...
import { followUserAction } from "../../actions/AuthAction";
...
const SimpleReel = ({ item }) => {
  ...
  const handleFollowUser = () => {
    dispatch(followUserAction(auth?.user?.id));
  };
      ...
      <div className="reel-wrapper">
        ...
        <div className="reel-user">
          <div className="user-details">
            <div
              className="username flex justify-start space-x-2"
              
            >
              <p onClick={() => navigate(`/profile/${item?.user?.id}`)}>{item?.user?.fullName}</p>
              <button
